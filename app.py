...
def add_player(name, email, phno, age):
    con = sqlite3.connect("Elitmus.db")
    c = con.cursor()
    try:
        x = (name, email, phno, age, 0, 0, 0)
        insert = """INSERT INTO Player (name, email, phno, age, start, stop, marks) VALUES (?,?,?,?,?,?,?);"""
        c.execute(insert, x)
    finally:
        con.commit()
        c.close()
...
def get_new_player():
    ...
    c = con.cursor()
    try:
        sql_select_query = "select max(id) from Player;"
...
def get_player(id):
    ...
    c = con.cursor()
    try:
        sql_select_query = "select * from Player where id = ?;"
...
def update_player():
    ...
    try:
        ...
        update = "UPDATE Player SET start = ?, stop = ?, marks = ? WHERE id = ?;"
        c.execute(update, x)
    finally:
        con.commit()
        c.close()
...
def get_players_played():
    ...
    c = con.cursor()
    try:
        sql_select_query = "select * from Player where start > 0 and stop > 0 order by id desc;"
        c.execute(sql_select_query)
        record = c.fetchall()
    finally:
        con.commit()
...
class Curr_User:
    def __init__(self):
        self.id = self.name = self.age = self.email = self.phno = self.start = self.end = self.marks = False
    def set_user(self):
    ...
    def set_marks(self, score):
        self.marks = score
    def time(self):
        return str(self.stop - self.start).split(":")
...
def login():
    if request.method == "POST":
        req = request.form
        if req["user"] == "Admin@Login" and req["pwd"] == "Admin@Pwd":
